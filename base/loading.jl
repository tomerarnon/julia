...
function get_uuid_name(project_toml::String, uuid::UUID, cache::TOMLCache = TOMLCache())
    project = parsed_toml(cache, project_toml)
    return get_uuid_name(project, uuid)
end

function collect_preferences!(project_toml::String, uuid::UUID, cache::TOMLCache = TOMLCache())
    ...
    for name in preferences_names
        ...
        if isfile(toml_path)
            prefs = parsed_toml(cache, toml_path)
...
function get_preferences(uuid::UUID, cache::TOMLCache = TOMLCache())
    ...
    for env in reverse(load_path())
        ...
        dicts = collect_preferences!(project_toml, uuid, cache)
